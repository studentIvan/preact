...
import * as React from '../../src/preact';

const flushQueue = [];
React.options.debounceRendering = (callback) => flushQueue.push(callback);

function EmptyComponent() {
	return <div/>;
}

// Basic adpater to make preact be somewhat compatible with the ReactDOM API as far as rerendering, unmounting and running in the test harness's event loop
const ReactDOM = {
	...
	unmountComponentAtNode(dom) {
		this.render(<EmptyComponent/>, dom);
		if (dom.firstChild) {
...
describe('ReactErrorBoundaries Compatibility', () => {
	let log;
	...
	beforeEach(() => {
		...
		BrokenComponentDidUpdate.defaultProps = {
			errorText: 'Hello'
		};
		...
		BrokenComponentWillUnmount.defaultProps = {
			errorText: 'Hello'
		};
	...
	it('does not swallow exceptions on updating without boundaries', () => {
		...
		expect(() => {
			// container.firstChild._component.setState({ foo: "bar" });
			ReactDOM.render(<BrokenComponentWillUpdate />, container);
		}).to.throw('Hello');
		...
		expect(() => {
			// container.firstChild._component.setState({ foo: "bar" });
			ReactDOM.render(<BrokenComponentWillReceiveProps />, container, container.firstChild);
		}).to.throw('Hello');
	...
	it('prevents errors from leaking into other roots', () => {
		...
		expect(container1.firstChild.textContent).to.be.equal('Before 1');
		expect(container2.firstChild).to.be.equal(null);
		expect(container3.firstChild.textContent).to.be.equal('Caught an error: Hello.');
		...
		expect(container1.firstChild.textContent).to.be.equal('After 1');
		expect(container2.firstChild.textContent).to.be.equal('After 2');
		expect(container3.firstChild.textContent).to.be.equal('After 3');
		...
		expect(container1.firstChild).to.be.equal(null);
		expect(container2.firstChild).to.be.equal(null);
		expect(container3.firstChild).to.be.equal(null);
	});
	...
	it('renders an error state if child throws in render', () => {
		...
		log.length = 0;
		ReactDOM.unmountComponentAtNode(container);
		expect(log).to.be.deep.equal(['ErrorBoundary componentWillUnmount']);
	});
	...
	it('resets callback refs if mounting aborts', () => {
		...
		expect(container.textContent).to.be.equal('Caught an error: Hello.');
		expect(log).to.be.deep.equal([
			'ErrorBoundary constructor',
			'ErrorBoundary componentWillMount',
			'ErrorBoundary render success',
	...
	if (React.createRef) {
		it('resets object refs if mounting aborts', () => {
			...
			expect(errorMessageRef.current.toString()).to.equal(
			...
			ReactDOM.unmountComponentAtNode(container);
			expect(log).to.be.deep.equal(['ErrorBoundary componentWillUnmount']);
			expect(errorMessageRef.current).to.equal(null);
		});
	}
	...
	it('successfully mounts if no error occurs', () => {
		...
		expect(container.firstChild.textContent).to.be.equal('Mounted successfully.');
		expect(log).to.be.deep.equal([
			'ErrorBoundary constructor',
			'ErrorBoundary componentWillMount',
			'ErrorBoundary render success',
	...
	it('picks the right boundary when handling unmounting errors', () => {
		...
		expect(container.textContent).to.be.equal('Caught an inner error: Hello.');
		expect(log).to.be.deep.equal([
	...
	it('can recover from error state', () => {
		...
		expect(container.textContent).not.to.contain('Caught an error');
		expect(log).to.be.deep.equal([
			'ErrorBoundary componentWillReceiveProps',
			'ErrorBoundary componentWillUpdate',
			'ErrorBoundary render success',
	...
	it('can update multiple times in error state', () => {
		...
		expect(container.textContent).to.be.equal('Other screen');
	...
	it('calls componentDidCatch for each error that is captured', () => {
		...
		expect(container.firstChild.textContent).to.be.equal(
	...
	it('discards a bad root if the root component fails', () => {
		...
		expect(err1.message).to.match(/got: null/);
		expect(err2.message).to.match(/got: undefined/);
	});
	...
	it('renders empty output if error boundary does not handle the error', () => {
		...
		expect(container.firstChild.textContent).to.be.equal('Sibling');
		expect(log).to.be.deep.equal([
		...
		log.length = 0;
		ReactDOM.unmountComponentAtNode(container);
		expect(log).to.be.deep.equal(['NoopErrorBoundary componentWillUnmount']);
	});
